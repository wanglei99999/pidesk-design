...
| [product-decisions.md](product-decisions.md) | 产品目标、范围和优先顺序 |
| [adr/README.md](adr/README.md) | 重要技术决定、原因和影响 |
...
| [07-learning-guide.md](07-learning-guide.md) | Pi 和 Agent 的学习与记录方法 |
| [06-development-rules.md](06-development-rules.md) | 安全基线和经过实现验证的长期规则 |

任务文件负责记录开发进度，场景文件负责说明为什么做和做到什么程度。两者不能相互替代。

## 基本规则

1. `main` 只保留已经完成并验证过的内容。
2. 一个开发任务对应一个任务文件和一个分支。
3. 分支名称使用 `task/NNN-short-name`。
4. 默认同一时间只推进一个主要任务。
5. 不在两台电脑上同时修改同一个分支。
...
| 已完成 | 验收标准满足，验证通过，已经合并到 `main` |

## 创建任务

1. 从 [tasks/TEMPLATE.md](tasks/TEMPLATE.md) 复制任务文件。
2. 在 [TASKS.md](TASKS.md) 登记编号、状态、分支和下一步。
3. 关联对应的场景文件。
4. 写清目标、不做什么和验收标准。
5. 创建 `task/NNN-short-name` 分支后开始开发。
...
git fetch --prune
git status --short
git switch task/NNN-short-name
git pull --ff-only
```
...
- 修改代码后立即运行相关检查；
- 产品目标、范围和优先顺序写入 `product-decisions.md`；
...
9. 开发者本人提交当前修改并推送任务分支；AI Agent 仅在得到明确授权时执行。
...
- 没有提交凭据、真实生产数据或无关文件；
- 按 Pull Request 模板完成验收、记录和公开仓库检查；
- 通过 Pull Request 合并到 `main`；
- [TASKS.md](TASKS.md) 已更新。

合并后删除远端任务分支。其他电脑再次开发前，先更新本地 `main`。
